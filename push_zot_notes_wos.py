...
with open(os.path.join(json_folder_path, "zot_notes_acos_oco.json")) as json_publications_file:
  zot_notes = json.load(json_publications_file)

with open(os.path.join(json_folder_path, "zot_pubs_acos_oco.json")) as json_publications_file:
...
note_delim = "<br />"
for zot_pub in zot_pubs:
  #print("zot item: "+zot_pub["key"])
  to_review = 1
  if len(zot_pub["data"]["tags"]):
    for tag in zot_pub["data"]["tags"]:
      if re.search("oco", tag["tag"]):
        to_review = 0
        break
  if not to_review:
    continue
  print("adding tag to "+zot_pub["data"]["title"])
  ...
  resp = zot.add_tags(zot_pub, 'source:oco', 'reviewer:asavtchenko')
  #resp = zot.add_tags(zot_pub, 'reviewer:asavtchenko')
  print(resp)
