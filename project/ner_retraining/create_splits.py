...
def build_dataset_id_to_papers(old_train_path, old_dev_path, old_test_path):
    ...
    citations_json = dev_citations_json #+ test_citations_json + train_citations_json
...
def get_old_splits(old_train_path, old_dev_path, old_test_path):
    ...
    #assert len(train_papers) + len(dev_papers) + len(test_papers) == 83, "There should be exactly 83 papers in the old splits"
...
def create_splits(dataset_id_with_papers_sorted):
    ...
    print("Num dev papers:", len(dev_papers))
    print("Num test papers:", len(test_papers))
    assert len(train_papers) + len(dev_papers) + len(test_papers) == 83, "There should be exactly 83 papers with datasets in them"
...
def main():
    ...
    new_splits_base_path = os.path.abspath(os.path.join("project", "data.new"))
