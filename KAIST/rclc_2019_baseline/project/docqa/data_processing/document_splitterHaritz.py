...
class MergeParagraphs(DocumentSplitter):
    ...
    def split(self, doc: List[List[List[str]]]):
        all_paragraphs = []
        print("flatten doc")
        flatten_doc = flatten_iterable(flatten_iterable(doc))
        print(flatten_doc)
        cnt = 0
        counted_words = 0
        start = 0
        text = []
        while counted_words < len(flatten_doc):
            text.append(flatten_doc[counted_words])
            cnt += 1
            counted_words += 1
            if self.max_tokens == cnt:
                all_paragraphs.append(ExtractedParagraph([text], start, counted_words))
                start = cnt
                text = []
                cnt = 0
        if cnt > 0:
