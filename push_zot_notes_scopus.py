...
with open(os.path.join(json_folder_path, "scopus_refs_2014-2020.json")) as json_publications_file:
  refs_list = json.load(json_publications_file)

with open(os.path.join(json_folder_path, "zot_notes_scopus.json")) as json_publications_file:
  zot_notes = json.load(json_publications_file)

with open(os.path.join(json_folder_path, "zot_pubs_scopus.json")) as json_publications_file:
...
for ref in refs_list:
  zot_pub = search_pubs(zot_pubs, ref["doi"])
  if not zot_pub:
    print('Cannot find '+ref["doi"]+' in zotero pubs')
    break
 ...
 #     if re.search("review", tag["tag"]):
  ...
  #print(note_item)
  datasets = get_dataset_list(eid_list, ref["datasets"])
  #print(datasets)
  #print(ref["dataset_dois"])
  note_item["note"] = note_delim.join(datasets)
  #.extend(ref["dataset_dois"]))
  note_item["tags"].append({"tag": "category:unknown"})
  note_item["tags"].append({"tag": "category:scopus"})
  #print(note_item)
 
  #resp = zot.add_tags(zot_pub, 'reviewer:'+dif_author)
  resp = zot.add_tags(zot_pub, 'source:scopus')
  print(resp)
